...
\begin{enumerate}[listparindent=\parindent]
    ...
    \item \textbf{Hoover (Gaussian)}
        ...
        The goal of the Gaussian thermostat is to ensure the change in instantaneous temperature $\Delta T$ is always 0 ($\Delta T \equiv 0$), which is accomplished by modifying the force calculation with the form $F = F_{interaction} + F_{Lagrange multiplier}$, where $F_{interaction}$ is the standard interactions calculated during the course of the simulation and $F_{Lagrange multiplier}$ keeps the kinetic energy constant.

...
%JIM: Good? I feel a little bad citing the Shell group, but it's a nice example of examining potential functions that coarse-grain out the solvent to different degrees, even including "local density" potential functions. It's maybe a little much, but provides good examples for the types of exotic force field terms I'm thinking about.
...
For a more in-depth discussion of common (and even less common) force field terms, see Ch. 4 of \citet{LeachBook}, or for an in-depth review of those specific to simulating biomolecules, see \citet{Ponder2003}.
...
\todo[inline, color={yellow!20}]{DLM: Should we mention ``bespoke'' versus general force fields, e.g. the idea that some people will parameterize each molecule like a unique snowflake, whereas others strive for generality and basically the ability to look up parameters? JIM: Does that cover it?  I essentially just talked a little about being wary of transferability, but maybe you're getting at something else?}.
...
Overall, \emph{none} of these deterministic thermostats are suitable for production-level simulations, except perhaps during an initial equilibration phase, because they do not sample from the canonical ensemble and lead to incorrect estimates of various computed properties [refs].
\todo[inline, color={yellow!20}]{DLM: Add refs.}

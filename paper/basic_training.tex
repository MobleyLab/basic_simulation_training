...
This includes not only macroscopic, bulk thermodynamic properties, such as density or heat capacity,
...
Paramount, however, is an understanding of the connection between thermodynamics and statistical mechanics, which allows us to relate macroscopic, experimental measurements to the results of the much smaller system that is simulated.
Involving many subtleties, this can be a confusing and difficult topic, and is presented in full within a number of excellent books.
\todo[inline, color={green!20}]{JIM: Citation for books by Shell, Dill, and maybe Hill?}
...
However, this is mainly a mathematical definition lacking intuition.
It is mostly useful in that it helps us to derive other mathematical relationships.
If we want to understand temperature from the point of view of a molecule and how it behaves, we finally must turn to statistical mechanics.
Since molecular dynamics is mostly used to simulate molecularly-detailed behavior, it is necessary to utilize statistical-mechanical expressions in computing what would be observed as the macroscopic, thermodynamic temperature.
...
This is of course useful because it allows us to assign molecular meaning to a well-defined thermodynamic process that can only be indirectly probed by experiment.
Importantly, as long as we have carefully defined our ensemble and thermodynamic path, we can apply the powerful relationships of thermodynamics to easily calculate many properties of interest simultaneously.
\todo[inline, color={green!20}]{JIM: I should think of a good example here. Also, I should mention that some thermodynamic properties are only well-defined for many molecules, not for single molecules or isolated molecules. This is a tricky discussion, though.} 
...
The reason for this is outlined in greater detail in ~\citep{Tuckerman:1992:}, but the basic idea is that the mathematical operator associated with an integrator must be unitary in order for phase space volume to be preserved.
If we define a unitary operator $\mathcal{L}$ that propagates coordinates $\vec{r}(t)$ and velocities $\vec{v}(t)$ (or any function depending on these quantities) over a time interval $\Delta t$

$$\mathcal{L} ( \Delta t, ( \vec{r}(t), \vec{v}(t) ) ) = ( \vec{r}(t + \Delta t), \vec{v}(t + \Delta t) )$$

then we may observe the following effect of also applying its inverse

$$\mathcal{L} ^ {-1} (\Delta t, \mathcal{L} (\Delta t, ( \vec{r}(t), \vec{v}(t) ) ) ) = ( \vec{r}(t), \vec{v}(t) )$$
...
To re-iterate what was discussed above, the timestep must be at least as small as an order of magnitude less than the smallest timescale of motion present in the system.

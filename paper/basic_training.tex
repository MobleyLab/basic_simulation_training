...
One way to visualize it is that entropy of a state is the \emph{volume} of 6N-dimensional phase space that the state occupies -  which in the one-dimensional case is just the \emph{width}.  
...
Sum of the short-range $\rho^{sr}$ and the long-range $\rho^{lr}$ charge distributions is still the same as the original charge distribution. 
